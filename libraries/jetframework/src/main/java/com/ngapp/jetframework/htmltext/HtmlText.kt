package com.ngapp.jetframework.htmltext
...
fun HtmlText(
    ...
    if (linkClicked != null) {
        ClickableText(
            ...
            onClick = { offset ->
                content
                    .getStringAnnotations(tag = URL_TAG, offset, offset)
                    .firstOrNull()
                    ?.let { stringAnnotation ->
                        Log.d("StringAnnotation", stringAnnotation.item)
                        linkClicked(stringAnnotation.item)
                    }
            }
        )
    } else {
